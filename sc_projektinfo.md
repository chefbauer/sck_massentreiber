...
| 2026-06-18 (session) | — | **Pin-Check JC1060P470**: GPIO5 = MIPI-DSI-Reset (KEIN freier Pin!); alter falscher Kommentar korrigiert; WS2812-Ring wurde darauf gelegt → Display zerstört | `schwippschwenker.yaml`, `sc_projektinfo.md` |
| 2026-06-18 (session) | — | **Beckenlicht reaktiviert**: `light_slots_becken` wieder aktiv auf GPIO3 (`pin_pwm3`); Aus-Button (✕) ergänzt | `lights.yaml`, `lvgl_basis.yaml` |
| 2026-06-18 (session) | — | **`c_tof_enable_mlx_distance: "50"`** – TOF-Schwelle für IR-Temp-Overlay per Substitution (50 mm) | `schwippschwenker.yaml`, `sensorphalanx.yaml` |
| 2026-06-18 (session) | — | **Standby öffnet Ventil**: `global_standby_open_ventil` (NVS, default false); Switch in Tab System; `bin_standby.on_press`→Ventil AUF / `on_release`→ZU; Not-Aus schließt Ventil | `schwippschwenker.yaml`, `schwenker.yaml`, `sensorphalanx.yaml`, `lvgl_basis.yaml` |
| 2026-06-18 (session) | — | **Block-Flag-System eingeführt**: `flag_standby_block_pump_auto` — Standby sperrt Pump-AUTO+ventil_auto+RV-Intervall; Pattern-Doku in `sc_arbeitsanweisungen.md` §9 | `schwenker.yaml`, `hardware.yaml`, `sensorphalanx.yaml`, `sc_arbeitsanweisungen.md` |
| 2026-06-18 (session) | — | **Ventil-Logik zentralisiert**: `ventil_auto` (500ms) als einziger Schreiber für `switch_ventil_zulauf`; Request-Flags (einmalig) + Block-Flags (dauerhaft); UI-Buttons setzen nur noch Flags; Not-Aus → `flag_blocked_ventil_auto` | `schwenker.yaml`, `hardware.yaml`, `sensorphalanx.yaml`, `lvgl_basis.yaml` |
